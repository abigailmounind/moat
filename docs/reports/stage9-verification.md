...
- 运行时将该纹理作为底层河面图层；现有 `frontend/src/map.js` 的三河路径、透明命中区、未来分叉和 `water-flow` 动态继续保留。资源设置 `pointer-events: none`，证明节点与河流入口仍可操作。
...
- 问题根因：上一版把水彩河面永久置于中心线路径遮罩中，最终态仍被裁切，造成硬切口、断河和零碎色块。
- 最终态修复：显现阶段仍由汇流点向左上、右上、下方推进；主河结束后切换完整遮罩，恢复用户 SVG 的原始轮廓。三条主河显现宽度为 170 / 160 / 176，支流为 58。
- 动态适配：三组流纹统一经过 river-alpha 遮罩，只在新河流真实透明区域内显示；旧 water-body / bank-edge 数量为 0。
- 边缘与色彩：仅在 18px 外轮廓带衰减高亮浅青像素，共调整 78,199 像素，保留内部水彩和白色流纹。色彩改为 brightness(.91) saturate(.78) contrast(1.04)。
- 验证：语法检查、项目检查、4 项状态测试、重新导出和 diff 检查通过。阶段 9 仍等待用户视觉验收。

## 2026-09-08 宽显现、细流纹与河尾延伸

- 显现遮罩主河宽度提升为 280 / 270 / 286，支流为 96，并分别加入 14px / 8px 柔化；主河推进延长至 2.45 秒，使用连续缓入缓出，完成遮罩在 2.95 秒接管最终轮廓。
- 每条主河由 4 条增至 6 条错位细流纹，共 18 条；周期由 20 秒调整为 14 秒，并提高分层可见度。流纹仍受新河流 alpha 遮罩、暂停控制和减少动态设置约束。
- 能力之河中心线延伸到 y=1100；新增由现有河岸算法生成的 watercolour tail，覆盖原图底缘并穿出 941 高度画布，缩放时不再露出截断端头。
- 浏览器无缓存检查确认：显现宽度 280 / 270 / 286、主河细流纹 18 条、流动周期 14 秒、河尾组件 1 个；1.05 秒中间帧三河仍连续从汇流区向外推进。阶段 9 继续等待用户视觉验收。

## 2026-09-08 顺河流纹与柔和汇流修订

- 撤销上一轮新增的下方水彩河尾、渐变和扰动滤镜，并恢复能力之河原有几何终点。
- 新增 flowLine 曲线生成：每个采样点依据河道切线计算法向偏移，18 条主河流纹因此沿各自河道弯曲，不再使用整体 translate 偏移。
- 河流遮罩前 45% 同步渐增 stroke-opacity；汇流高光由 0.35 倍尺寸和透明状态经过 1.15 秒缓慢显现，避免中心首帧突现。主河推进调整为 2.65 秒，完整轮廓在 3.15 秒接管。
- 无缓存浏览器 220ms 采样：三河显现透明度约 0.084 / 0.004 / 0，汇流高光 opacity 约 0.188；18 条主河流纹均无 transform 属性；河尾节点和相关 defs 均为 0。阶段 9 继续等待用户视觉验收。

## 2026-09-08 流向、河尾与文字精简候选

- 所有流纹路径增加 `core-to-outlet` 方向约束，并以 `pathLength=100` 标准化；三条主河的几何点序均由共同核心向各自河尾排列。
- 主河流纹由每河 6 条减为 4 条，缩短亮纹并降低线宽；支流保留每支流 2 条低对比纹理。暂停、页面隐藏和减少动态效果继续沿用原控制。
- 完整河面遮罩从 3.15 秒瞬时接管改为 2.75 秒开始、持续 1.45 秒的柔和交叠，使河尾与水彩边缘渐进补全。
- 河流标签底部重复的证明数量 / 状态和两处装饰性地图批注已隐藏；关键河名、语义、节点、演示标记与文字等价入口保留。
- 自动化与静态检查用于确认结构和回归，不能替代流向、河尾质感与文字密度的用户视觉验收。

## 2026-09-09 未来分叉水系统一

- 用户确认河尾显现、文字密度、河流摘要和证明详情可以接受；其他页面细节留待客户端结合图片标记继续讨论。
- 用户指出未来分叉使用的单层 SVG 描边与主河水彩 SVG 不属于同一视觉语言。本轮保留候选方向几何、从属层级和交互状态，将单层硬描边替换为柔和岸缘、低饱和青蓝水体、浅色水洗与细微顺河纹理四层水道。
- 分叉使用轻度 SVG 扰动形成有机边缘，展开仍沿原路径显现；减少动态效果时直接显示完整末态。用户提供的主河 SVG、地形、页面布局、摘要和证明详情均未修改。
- `npm test` 九个测试套件、`npm run check` 和河流图层重新导出通过。临时 Playwright 已安装在 `/tmp`，但 Chromium 下载被服务器反复中断，因此本轮仍需用户在现有本地端口确认分叉视觉；阶段 9 尚未标记视觉验收完成。

## 2026-09-09 标注反馈修订：纹理、证明文字与能力分叉

- 主河视觉资产仍为内嵌位图，无法直接读取独立流纹组件。本轮依据用户红框指出的浅色水纹方向，为三条主河各手工描摹 4 条独立曲线；主河动态不再由中心线法向平移生成。支流保留低对比中心线纹理。
- 证明节点默认只显示圆点；对应证明打开后才显示说明文字。说明使用暖纸半透明底、深灰绿文字与短淡入，按钮的无障碍名称始终保留。
- 能力河候选分叉从下游外岸向左下展开，避开右侧详情面板和证明标签；水道采用渐窄填充轮廓、柔和岸层、青蓝主体、浅色水洗与内部动态浅纹。
- 浏览器复核：默认态三个证明说明均为 hidden / opacity 0；打开能力证明时对应说明出现；未来态重新关闭证明说明。12 条主河纹理均标记为 painted-river-trace，动画为 running，路径方向为共同核心到河尾。
- 36 项 Node 测试、项目静态检查、SVG 重新导出和 diff 格式检查通过。以上仍需用户视觉确认，不标记阶段 9 验收完成。

## 2026-09-09 常驻流纹撤销与真实河岸接入

- 用户澄清：删除代码额外生成的主河、支流和未来分叉常驻动态流纹；用户河流 SVG/PNG 内部原有的静态白色水纹完整保留。
- 透明度轮廓实测显示，能力河在 y=720 附近的可见水体约从 x=887 开始，旧几何骨架接入点 x=815 位于河外，因此上一版出现明显断口。
- 新分叉从水体内部 x=910, y=720 开始，穿过真实左岸后向左下展开；起点与主河重叠，末端渐窄。去掉浅青外缘，采用更接近主河的深青蓝、低透明水洗和轻颗粒。
- 运行时核验：water-flow 与 future-water-flow 元素数量均为 0；源河面静态纹理仍由 rivers-watercolor-hires-clean.svg 提供。浏览器截图确认分叉与能力河连续，无上一版空白断口。
- 36 项测试、静态检查、SVG 导出和 diff 格式检查通过。分叉最终质感仍等待用户视觉验收。


## 2026-09-09 取消地图分叉可视化

- 用户最终决定停止在「我的河」中生成未来分叉：水道即使接入河岸，仍呈现贴附感，无法与原始水彩河流稳定融合。
- 已移除未来水道、地图分叉标签、方向几何路径与相关样式。打开「未来方向」现在只更新右侧详情，三条主河、支流和原始静态河面纹理均不变化。
- 未来方向数据、支持 / 缺口 / 验证行动，以及独立「未来路径」工作区继续保留；取消的仅是地图分叉可视化。
- 「带光泽的横向引导线 + 文字」记录为后续可选方案，本轮不实现，需另行视觉验证与用户确认。
- 自动检查负责确认运行时不再输出分叉水道或标签；阶段 9 整体仍待用户视觉验收。
