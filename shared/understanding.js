...
export function validateUnderstandingProposal(value,{inputIds=[],skippedTopics=[]}={}){
 ...
 const trace=item=>{if(item.rule_id!==undefined&&(!/^[A-Z][A-Z0-9_-]{2,31}$/.test(item.rule_id)||item.rule_version!=='0.1'))errors.push(item.id+':rule_trace');if(item.basis_refs!==undefined&&(!Array.isArray(item.basis_refs)||new Set(item.basis_refs).size!==item.basis_refs.length||item.basis_refs.some(ref=>!item.input_refs?.includes(ref))))errors.push(item.id+':basis_refs');};
 ...
 each('claims',item=>{refs(item);trace(item);if(!['situation','experience','preference','constraint'].includes(item.kind)||item.source_type!=='ai_proposal'||item.confirmation_status!=='pending'||typeof item.text!=='string'||!item.text.trim())errors.push(item.id+':claim_shape');});
 each('evidence_drafts',item=>{refs(item);trace(item);if(item.source_type!=='user_self_report'||item.confirmation_status!=='pending'||typeof item.title!=='string'||!item.title.trim()||typeof item.experience!=='string'||!item.experience.trim()||!Array.isArray(item.actions)||!item.actions.length||item.actions.some(action=>typeof action!=='string'||!action.trim())||!(typeof item.result==='string'||item.result===null)||!(typeof item.source==='string'||item.source===null)||!Array.isArray(item.limitations)||item.limitations.some(x=>typeof x!=='string'||!x.trim()||x.length>1000))errors.push(item.id+':evidence_shape');});
 const evidenceIds=new Set((value?.evidence_drafts??[]).map(item=>item?.id));
 each('capital_links',item=>{refs(item);trace(item);if(!capitals.has(item.capital)||!evidenceIds.has(item.evidence_id)||item.confirmation_status!=='pending'||typeof item.aspect!=='string'||!item.aspect.trim()||typeof item.explanation!=='string'||!item.explanation.trim())errors.push(item.id+':capital_link_shape');});
 each('river_links',item=>{refs(item);trace(item);if(!rivers.has(item.river)||!evidenceIds.has(item.evidence_id)||item.confirmation_status!=='pending'||typeof item.explanation!=='string'||!item.explanation.trim())errors.push(item.id+':river_link_shape');});
 each('future_direction_drafts',item=>{refs(item);trace(item);if(!rivers.has(item.river)||item.confirmation_status!=='pending'||typeof item.name!=='string'||!item.name.trim()||typeof item.support!=='string'||!item.support.trim()||typeof item.unknown!=='string'||!item.unknown.trim()||typeof item.next_action!=='string'||!item.next_action.trim())errors.push(item.id+':direction_shape');});
