...
test('同步页面先预览再确认，取消不提交，失败保留入口，重试期间拒绝重复操作',async()=>{
 const previous=Object.getOwnPropertyDescriptor(globalThis,'document'),original={...connection};
 ...
 try{
  ...
  const pending=click('retry');await click('retry');assert.equal(calls,2);
