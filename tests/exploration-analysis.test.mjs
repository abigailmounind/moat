...
const ui={answers:{situation:'change',situationOther:'',blockers:[],experience:'project',actions:['organize'],actionOther:'',outcomes:['artifact'],source:''}};
...
test('运行时契约拒绝第四条河、悬空引用和重复 ID',()=>{const proposal=structuredClone(session());const value=runSyntheticAnalysis(proposal);value.river_links[0].river='luck';assert.equal(validateUnderstandingProposal(value,{inputIds:['q1','q2','q3','q4','q5']}).ok,false);const missing=structuredClone(value);missing.capital_links[0].evidence_id='missing';assert.equal(validateUnderstandingProposal(missing,{inputIds:['q1','q2','q3','q4','q5']}).ok,false);const duplicate=structuredClone(value);duplicate.unknowns[0].id=duplicate.evidence_drafts[0].id;assert.equal(validateUnderstandingProposal(duplicate,{inputIds:['q1','q2','q3','q4','q5']}).ok,false);});
