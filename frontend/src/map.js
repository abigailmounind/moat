...
export function mapMarkup(directions){
 ...
 const revealStrokes=mainReveal.map((d,i)=>`<path class="river-reveal-stroke reveal-${waterways[i].id}" data-reveal-direction="core-to-outlet" d="${d}" pathLength="100" fill="none" stroke="#fff" stroke-width="${[280,270,286][i]}" stroke-linecap="round" stroke-linejoin="round" style="--reveal-delay:${[.12,.20,.28][i]}s"/>`).join('');
 ...
 const accents={love:'#b87562',survival:'#738564',ability:'#77778d'};
 const highlightPaths={
  love:[
   'M916 480 C873 463 871 415 833 395 C790 372 740 388 703 355 C675 330 690 297 652 276 C605 250 586 230 568 198 C547 158 509 150 490 125 C464 94 420 92 385 91 C341 90 309 57 269 32'
  ],
  survival:[
   'M938 479 C969 442 1019 449 1061 431 C1110 411 1121 378 1131 340 C1142 294 1180 268 1227 273 C1274 279 1323 288 1370 276 C1421 265 1456 243 1453 200 C1449 161 1490 146 1519 129 C1555 108 1563 77 1526 39'
  ],
  ability:[
   'M934 508 C957 548 906 563 880 590 C852 622 864 656 905 675 C949 695 975 711 978 750 C983 798 949 825 907 846 C853 873 811 897 817 941'
  ]
 };
 const systems=waterways.map(r=>`<g class="river-system river-system-${r.id}" data-system="${r.id}" style="--river-accent:${accents[r.id]}">${highlightPaths[r.id].map((d,index)=>`<path pathLength="100" class="river-highlight river-highlight-halo ${index?'river-highlight-branch':''}" mask="url(#river-alpha)" d="${d}" fill="none" stroke="#fff1b8" stroke-width="${index?12:18}" stroke-linecap="round" stroke-linejoin="round"/><path pathLength="100" class="river-highlight river-highlight-line ${index?'river-highlight-branch':''}" mask="url(#river-alpha)" d="${d}" fill="none" stroke="#fffbed" stroke-width="${index?1.5:2}" stroke-linecap="round" stroke-linejoin="round"/>`).join('')}<path class="river-hit" data-hover-river="${r.id}" data-select-river="${r.id}" d="${center(r.knots)}" fill="none" stroke="transparent" stroke-width="96"/></g>`).join('');
 return `<svg class="river-map" viewBox="0 0 1672 941" aria-hidden="true" focusable="false"><defs><radialGradient id="core-glow"><stop stop-color="#fffced" stop-opacity=".82"/><stop offset="1" stop-color="#fffced" stop-opacity="0"/></radialGradient><mask id="river-reveal" maskUnits="userSpaceOnUse" x="0" y="0" width="1672" height="941"><rect width="1672" height="941" fill="#000"/>${revealStrokes}${tributaryReveal}<rect class="river-reveal-complete" width="1672" height="941" fill="#fff"/></mask><mask id="river-alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="1672" height="941" style="mask-type:alpha"><image href="/assets/three-rivers/rivers-watercolor-hires-clean.svg" x="0" y="0" width="1672" height="941" preserveAspectRatio="none"/></mask><radialGradient id="mist-fill"><stop stop-color="#fffdf4" stop-opacity=".93"/><stop offset=".52" stop-color="#fffdf4" stop-opacity=".62"/><stop offset="1" stop-color="#fffdf4" stop-opacity="0"/></radialGradient></defs><g class="watercolor-reveal" mask="url(#river-reveal)"><image class="watercolor-rivers" href="/assets/three-rivers/rivers-watercolor-hires-clean.svg" x="0" y="0" width="1672" height="941" preserveAspectRatio="none" aria-hidden="true" focusable="false"/></g><circle class="confluence" cx="884" cy="452" r="31" fill="url(#core-glow)"/> <g class="unknown-mist"><ellipse class="mist-breath" cx="390" cy="439" rx="210" ry="90" fill="url(#mist-fill)"/><ellipse class="mist-breath" cx="1420" cy="358" rx="225" ry="98" fill="url(#mist-fill)"/><ellipse class="mist-breath" cx="1220" cy="812" rx="195" ry="91" fill="url(#mist-fill)"/></g><g class="ambient-mist"><ellipse cx="1420" cy="130" rx="260" ry="92" fill="url(#mist-fill)"/><ellipse cx="290" cy="820" rx="220" ry="100" fill="url(#mist-fill)"/><ellipse cx="1515" cy="720" rx="200" ry="100" fill="url(#mist-fill)"/></g><g class="arrival-mist"><ellipse cx="550" cy="215" rx="530" ry="410" fill="url(#mist-fill)"/><ellipse cx="1270" cy="350" rx="450" ry="450" fill="url(#mist-fill)"/><ellipse cx="875" cy="820" rx="540" ry="380" fill="url(#mist-fill)"/></g>${systems}</svg>`;
}
