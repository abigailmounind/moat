...
for(const button of [...document.querySelectorAll('[data-card-value=confirmed]')])p.click(`[data-card="${button.dataset.card}"][data-card-value=confirmed]`);
p.click('[data-next]');p.click('[data-next]');assert.match(p.q('.explore-card').textContent,/<我的项目经历>/);assert.equal(document.querySelector('.explore-card img'),null);p.click('[data-next]');p.click('[data-next]');p.click('[data-apply-update]');await new Promise(resolve=>setTimeout(resolve,0));assert.match(p.q('h1').textContent,/已保存/);let raw=p.raw();p.close();
p=await page('app','?view=river',raw);assert.match(p.q('.top-tools').textContent,/关于地图/);p.click('#river-ability');assert.match(p.q('#preview').textContent,/另一种任务/);p.click('#preview [data-future]');const bridge=p.q('#detail-panel a[href*="view=paths"][href*="river=ability"]').getAttribute('href');raw=p.raw();p.close();
